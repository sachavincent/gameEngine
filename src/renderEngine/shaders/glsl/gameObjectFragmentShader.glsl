...
void main() {
    ...
    if (color.x == -1) {
        finalColor = texture(modelTexture, pass_textureCoords, -1.0);
        if (finalColor.a < 0.5) {
            discard;
        }
    } else {
    ...
    out_Color = vec4(totalDiffuse, 1.0) * finalColor + vec4(totalSpecular, 1.0);
    out_Color = mix(vec4(skyColor, 1.0), out_Color, visibility);
    //        out_Color.xyz = min(out_Color.xyz, .8);
