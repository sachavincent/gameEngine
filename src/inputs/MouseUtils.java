...
//                    currentTerrainPoint.y = 2 * (float) Math.floor(currentTerrainPoint.y / 2) + 1;
                    ...
                    System.out.println(Camera.getInstance().getDirection());
...
//                            terrain.placeItem(abstractInsula2, currentTerrainPoint);
        ...
        callback3 = GLFW.glfwSetKeyCallback(window, (w, key, scancode, action, mods) -> {
            Vector2f cursorPos = getCursorPos();

        });

        callback4 = GLFW.glfwSetCursorPosCallback(window, (w, xPos, yPos) -> {
